...
if( ! class_exists('Pvalley_User_Role_Based_Shipping_Manage_Shipping') ) {
	...
	final class Pvalley_User_Role_Based_Shipping_Manage_Shipping {
		...
		public function __construct() {
			...
			add_filter("woocommerce_package_rates", array($this, "hide_shipping_methods"), 999);
		}

		/**
		 * Hide shipping methods based on the selected option.
		 *
		 * @param $rates
		 *
		 * @return array|mixed
		 */
		public function hide_shipping_methods($rates) {
			$show_only = ! empty(self::$settings['show_only']) ? self::$settings['show_only'] : "";
			switch ($show_only) {
				case "free_shipping":
					return $this->show_only_free_shipping( $rates );
				default:
					return $rates;
			}
		}
		...
		/**
		 * Hide all the shipping methods other than Free Shipping method.
		 *
		 * it will be applicable only when Free Shipping is available.
		 *
		 * It won't hide any shipping methods when there is no Free Shipping methods.
		 *
		 * @param $rates
		 *
		 * @return array|mixed
		 */
		public function show_only_free_shipping($rates) {
			$free = array();
